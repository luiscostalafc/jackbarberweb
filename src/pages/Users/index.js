		...
		() => [
			...
			},
			{
				...
				cell: row => (
					<>
						{userType === 'providers' && (
							<ButtonBlock to={`/admin/schedules/${row.id}`} green>
								Agenda
							</ButtonBlock>
						)}
						<Button
	...
	return (
		<Container>
			<Card>
				<DataTable
					title={
						userType === 'users' ? 'Lista de Usuários' : 'Lista de Prestadores'
					}
					columns={columns}
					data={users}
					pagination
				...
				/>
				{userType === 'providers' && (
					<ButtonBlock to="/admin/users/create">Adicionar</ButtonBlock>
				)}
			</Card>
		</Container>
	);
